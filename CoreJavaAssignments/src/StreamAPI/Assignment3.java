...
2) find out how many players have scored more than 5000 runs.
	...
	hint:- use "filter" to check players scored more than 5000 runs and "count" method to count them.
		...
		playerList.stream().filter((s)->s.getCountryname().equalsIgnoreCase("india")).forEach((s)->System.out.println(s.getName()));
		System.out.println("players who have scored more than 5000 runs are only "+playerList.stream().filter((s)->s.getRuns()>5000).count());
